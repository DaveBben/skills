...
Vendored from <https://agentskills.io/specification.md>, fetched 2026-08-27. This is a local copy so agents can read it without network access. Check the source for the current version before relying on any detail.
...
The agent loads this entire file once it decides to activate a skill. Consider splitting longer `SKILL.md` content into referenced files.

## Optional directories

A skill directory may contain any files and directories beyond the required `SKILL.md`. The conventions below are recommendations.
...
Agents load skills *progressively*, pulling in more detail only as a task calls for it. Structure skills to take advantage of this:
