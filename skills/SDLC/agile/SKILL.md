...
* **Fixed authorship.** The agent proposes the cards and the user accepts them. The user writes the acceptance criterion. The agent writes everything else.
...
Every stop in this skill ends with one line: the branch, the last commit, which rows are red, and what the agent does on "go" with the paths it touches. The stops are both halts here, the red commit in section 4, and the merge in section 7.
...
* **Then stop,** with the stop line. Write no implementation until the user says go, and change no accepted row without saying so.
...
**Merge description.** Before the merge, the user writes the merge description in their own words, before they read the builder's report or the review's Done block: what changed, and one line per new function or branch saying what it is for. It is the merge request's body. Do not draft it. The review returns its Done block; print it only after the description exists.
...
* **The user merges.** Present the description, the Done block and the diff, then the stop line. The agent never merges to main. One slice, one merge. Delete the branch after the merge and clear the red-commit guard.
...
* **Open a merge request when the repo has a remote.** The merge description is its body; the Done block follows it. The user merges, after any reviewer the repo requires.
