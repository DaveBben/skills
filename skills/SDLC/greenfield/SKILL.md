...
* Run the `orient` skill to write `AGENTS.md`, with `CLAUDE.md` symlinked to it. It is the file every later session reads first; when the template shipped an instructions file, `orient` shows the mapping and asks before rewriting it.
