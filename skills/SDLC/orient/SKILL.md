---
name: orient
description: "Use this skill whenever an agent needs to orient itself in a repository, or a repository needs the one file every agent session reads first written or revised: AGENTS.md, with CLAUDE.md symlinked to it. It carries the product charter (what the product is, for whom, why, and what it is not) and the agent instructions (stack, commands, hard constraints, pointers). Use it on: 'orient yourself', 'orient yourself with this repo', 'familiarize yourself with this codebase', 'get to know this repo', 'setup claude in this repo', 'set up claude for this project', 'get this repo ready for ai agents', 'make this repo agent ready', 'setup initial context', 'create repo context', 'write the charter', 'create a charter', 'project charter', 'what is this project for', 'document the vision', 'define what this product is', 'what's in and out of scope for this project', 'set the boundaries', 'write AGENTS.md', 'write CLAUDE.md', 'set up CLAUDE.md', 'create an agents file', 'streamline my CLAUDE.md', and when a project has no AGENTS.md. Use it on the bare words 'charter' or 'orient' alone. When the file exists and is sound, orienting means reading it and reporting; otherwise interview the user for the charter, fill the stack and commands from the repository, write AGENTS.md in five sections under 100 lines and symlink CLAUDE.md to it; when an instructions file already exists, show the mapping and ask once before rewriting. Finish by offering `harness` for the checks and hooks. Do not use it for linting, hooks, commit gates or guardrails; that is `harness`. Do not use it to specify a feature or a change; that is `agile`."
license: MIT
...
The charter is not a requirements document. It holds no feature list, no priorities, no metrics, no architecture. Those live in slices, tests and ADRs. A charter that grows past thirty lines is carrying something that belongs elsewhere.
...
* **Cap it at 100 lines,** the charter sections under thirty of them. Every line costs context on every turn. A module's conventions belong in a nested instructions file in that module; a rule for one file type belongs in a path-scoped rule. The `harness` skill wires those.
* **Symlink `CLAUDE.md` to it.** One file, every tool reads it. Symlink any other conventional name the repository already carries the same way.
