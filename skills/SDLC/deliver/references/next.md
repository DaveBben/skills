...
Inputs: the stories and spikes of one feature, with their blockers. Read them where the `Backlog:` line of `AGENTS.md` says stories live, with the access method it lists. On a tracker they are the epic's children, their rank and their blocking links. With `Backlog: none` or no such line, they are the `Stories:` list in the feature header of `docs/delivery/{slug}.md`. When `AGENTS.md` names a per-change spec directory, the log is the bottom of that change's spec instead. From the feature log, read only the `Stories:` and `Parked:` lines, and grep for unpinned `Learned` lines, `Observed` lines and `Proposed refactor` lines marked "open". A `Learned` line can change what a story needs. An `Observed` line shows what people did with a shipped story, and can make a later story unneeded or more urgent. Report either beside the pick.
