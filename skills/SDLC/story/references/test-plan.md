...
Loaded by the `story` skill when the tests for a change must be listed, or an existing set of tests checked for gaps. When the change has criteria, the user has confirmed them first.

Propose the tests for one change as a table. Run alone, the user cuts the rows they do not want and adds any they do; a row not cut is accepted, and no test is written until the table has been in front of the user. Run from `deliver`'s setup subagent, which calls the `story` skill for the table after the user confirms the criteria, the agent applies the cut rules itself, writes the tests, and the table goes into the pull request description; a row that exposes a product decision is the one thing that still stops for the user.

Inputs: the change's acceptance criteria and the code the change touches. Read seams and fields off the existing code, not off a design document. When a new change has no criterion, or one offered names no observable outcome, write the criterion the change implies with the rules in the `story` skill, put it to the user in one line, and propose no rows until it is confirmed. When the code the change touches cannot be found, name the files searched and ask which ones to read.
...
* **Seam:** one row per boundary crossed: database, queue, third-party API, process edge. Pin the contract against a recorded exchange when the real dependency is unreachable. Where an ADR stands a twin in for a third-party service (a fake the repository keeps under `tests/twins/<service>/`, checked against recorded real responses), the row drives the twin, and a row the twin's contract suite lacks is added there too.
...
## Check existing tests for gaps

When the user asks whether a change is covered, run the generators above against its criteria, or against the behaviours the code shows when it has none, and compare the rows they produce with the tests that exist. Report each row with no test, and each existing test that no generator or criterion accounts for. Cut rules apply to both lists. Change no test until the user has seen the list.
