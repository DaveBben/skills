...
* **Write a flag as criteria** when the story merges before the feature is complete and exposes half of it, or backing it out needs more than a `git revert`: with the flag off, the person sees today's behaviour. The story that completes the behaviour gets a criterion that removes the flag. Acceptance tests turn the flag on explicitly, and the flag-off criterion's test turns it off.
